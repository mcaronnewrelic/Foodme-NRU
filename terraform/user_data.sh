...
# Nginx config - Proxy ALL requests to Node.js for Angular routing
cat > /etc/nginx/conf.d/foodme.conf << 'EOF'
server {
    listen 80;
    server_name _;
...
# Static index.html not needed - Node.js serves index.html via catch-all handler
# All requests are proxied to Node.js which serves Angular app and handles routing
echo "Nginx configured to proxy all requests to Node.js - no static index.html needed"
